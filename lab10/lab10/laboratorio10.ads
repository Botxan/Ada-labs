package Laboratorio10 is
   -- Es posible que alguno de los elementos aqui especificados sea erroneo
   -- Si encuentras algun tipo de error, avisa a traves del foro de la practica
   -- para que todo el mundo este al tanto del problema.

   -- Si necesitas mas subprogramas, no hay problema, anadelos en un paquete aparte
   -- para tu uso personal. Si los subprogramas de esta especificacion necesitan de
   -- otros subprogramas auxiliares, los puedes anadir NO AQUI sino en la parte
   -- de implementacion del paquete (asi estarian disponibles solo para tus programas,
   ...
   --Post: Resultado=true <-> Img es una imagen sin dudas
   ...
   procedure Buscar_Pista(Lp: in T_Lista_E_Pistas; Img: in T_Imagen;
      P: out T_Pista);
   --Post: P es una pista de LP que se puede aplicar en Img
